# Atlas — autonomous AI agent for your computer

Atlas sees your screen, drives the mouse/keyboard, controls a real browser, runs shell
...
**macOS:** double-click **`Atlas.command`**
**Windows:** double-click **`Atlas.bat`**
...
Turn Atlas into a double-clickable app that needs **no Python and no terminal** to run:

- **macOS:** double-click **`BUILD_DESKTOP_APP.command`** → produces **`dist/Atlas.app`**.
  Drag it to Applications. (Uses PyInstaller — completely free.)
- **Windows:** run `python -m PyInstaller --noconfirm Atlas.spec` → produces `dist/Atlas/Atlas.exe`.
...
- **Smarter web use:** Atlas prefers DOM browser controls, re-checks pages after navigation, handles
...
- **Screen Recording** — so Atlas can see the screen
...
## 📱 Control your PC from your phone (Atlas Link)

Ask Atlas to “start remote control,” or it runs the `start_remote_control` tool. You’ll get a
**URL + 4-digit PIN**. On your phone (same Wi-Fi) open the URL, enter the PIN, and you get
**Atlas Link**: a faster mirrored screen with tap, click-and-drag, fullscreen mirror,
trackpad, keyboard, quality controls, and a Chat tab for telling Atlas what to do remotely.
It’s LAN-only and PIN-gated; stop it when done.

---

## 🧠 What makes Atlas accurate
...
- **“Chat history corrupted / desynced”** — fixed; if you ever see it, just resend (Atlas now
  auto-clears the bad turn).
- **Clicks do nothing** — grant Accessibility, then fully quit and reopen Atlas.
...
| File | Purpose |
|---|---|
| `Atlas.command` / `Atlas.bat` | one-click run |
| `BUILD_DESKTOP_APP.command` | build standalone `Atlas.app` |
| `main.py` | entry point |
...
| `remote_server.py` | Atlas Link phone control |
